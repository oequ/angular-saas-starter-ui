...
6. **Billing bridge (`0008`, `apps/web`):** `organizations.plan_id` drives `seats_limit` (free→3, pro→10, team→50). `get_organization_billing_snapshot` is authoritative for `seatsUsed` / cap in UI; mock paywall checkout/downgrade calls `update_organization_plan`. Stripe webhooks deferred.
...
- Seat caps are enforced in Postgres; plan changes on `apps/web` sync the cap via `0008`. Stripe checkout/webhooks remain mock until phase 3.
